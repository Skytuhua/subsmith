...
export function parseVtt(input: string): ParseResult {
  ...
  if (!/^﻿?WEBVTT/.test(text)) {
    warnings.push({ message: 'File does not start with the "WEBVTT" signature.' })
  }
  ...
  for (const block of blocks) {
    ...
    if (!hasTiming) {
      ...
      if (!seenCue && (isHeaderBlock(trimmed) || headerBlocks.length === 0)) {
        ...
        warnings.push({ message: `Skipped a non-cue block after cues began: "${truncate(trimmed)}"` })
      }
      continue
    }
    ...
    if (start === null || end === null) {
      warnings.push({ message: `Skipped a cue with an unparseable timecode: "${lines[timingIdx]}"` })
      continue
    }
  ...
  const vttHeader = headerBlocks.length > 0 ? headerBlocks.join('\n\n') : 'WEBVTT'
