...
export function parseSrt(input: string): ParseResult {
  ...
  for (const block of blocks) {
    ...
    if (timingIdx === -1) {
      warnings.push({ message: `Skipped a block with no timing line: "${truncate(trimmed)}"` })
      continue
    }
    ...
    if (start === null || end === null) {
      warnings.push({ message: `Skipped a block with an unparseable timecode: "${lines[timingIdx]}"` })
      continue
    }
