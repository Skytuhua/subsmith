import type { AssEvent, Cue, ParseResult, ParseWarning, Subtitle } from '../types'
...
export function parseAss(input: string): ParseResult {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (ev) {
      ...
      rawEvents.push({
        kind: (ev[1][0].toUpperCase() + ev[1].slice(1).toLowerCase()) as 'Dialogue' | 'Comment',
        rest: ev[2],
        lineNo: i,
      })
    }
  ...
  if (startIdx === -1 || endIdx === -1 || textIdx === -1) {
    warnings.push({ message: 'ASS [Events] Format is missing Start/End/Text; using defaults.' })
  }
  ...
  for (const raw of rawEvents) {
    ...
    if (start === null || end === null) {
      warnings.push({ line: raw.lineNo + 1, message: `Skipped an event with bad timing: "${startStr} / ${endStr}"` })
      continue
    }
...
function pick(fields: string[], format: string[], key: string, fallback: string): string {
...
export function synthHeader(format: string[] = DEFAULT_ASS_EVENT_FORMAT): string {
  return [
    '[Script Info]',
    'ScriptType: v4.00+',
    'WrapStyle: 0',
