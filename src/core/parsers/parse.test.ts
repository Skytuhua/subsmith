...
describe('SRT parsing', () => {
  it('parses cues with multi-line text', () => {
    ...
    expect(subtitle.cues[0]).toMatchObject({ start: 1000, end: 4000, text: 'Hello world\nsecond line' })
    expect(subtitle.cues[1]).toMatchObject({ start: 5000, end: 6500, text: 'Next cue' })
  })
...
describe('ASS parsing', () => {
  ...
  it('preserves the header and round-trips events', () => {
    ...
    expect(out).toContain('Dialogue: 0,0:00:01.00,0:00:04.00,Default,,0,0,0,,Hello world\\Nsecond line')
    expect(out).toContain('Comment: 0,0:00:05.00,0:00:06.50,Default,,0,0,0,,Next cue, with comma')
  })
})
