...
export function detectEncoding(bytes: Uint8Array): DetectedEncoding {
  ...
  let bin = ''
  for (let i = 0; i < sample.length; i += 1) bin += String.fromCharCode(sample[i])
...
export function detectAndDecode(bytes: Uint8Array, override?: string): LoadedText {
  if (override) {
    return { text: decodeText(bytes, override), encoding: override, confidence: 1, fromBom: false }
  }
