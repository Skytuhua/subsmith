...
import { findReplace, stripTags, trimWhitespace, removeEmpty, sortByTime, fixMojibakeAll } from './text'
...
describe('shift', () => {
  it('moves all cues by a signed offset', () => {
    const s = shift(sub([[1000, 2000, 'a'], [3000, 4000, 'b']]), -500)
    expect(s.cues.map((c) => [c.start, c.end])).toEqual([[500, 1500], [2500, 3500]])
  })
  it('can scope to a predicate', () => {
    const s = shift(sub([[1000, 2000, 'a'], [3000, 4000, 'b']]), 1000, (_c, i) => i === 1)
...
describe('two-point linear sync', () => {
  ...
  it('corrects progressive drift (scale + offset)', () => {
    ...
    const s = twoPointSync(sub([[1000, 2000, 'a'], [61000, 62000, 'b']]), 1000, 1000, 61000, 60000)!
    expect(s.cues[0].start).toBe(1000)
...
describe('text operations', () => {
  it('find & replace literal, counting matches', () => {
    const { subtitle, count } = findReplace(sub([[0, 1, 'la la la'], [1, 2, 'la']]), 'la', 'LA')
    expect(count).toBe(4)
    expect(subtitle.cues[0].text).toBe('LA LA LA')
  })
  it('find & replace regex with backreference', () => {
    const { subtitle, count } = findReplace(sub([[0, 1, 'John Smith']]), '(\\w+) (\\w+)', '$2 $1', {
      regex: true,
    })
    expect(count).toBe(1)
  ...
  it('removes empty cues', () => {
    const s = removeEmpty(sub([[0, 1, 'a'], [1, 2, '   '], [2, 3, 'b']]))
    expect(s.cues).toHaveLength(2)
  })
  it('sorts by start time stably', () => {
    const s = sortByTime(sub([[3000, 4000, 'c'], [1000, 2000, 'a'], [2000, 2500, 'b']]))
