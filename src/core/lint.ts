...
export function lint(sub: Subtitle, thresholds: LintThresholds = DEFAULT_THRESHOLDS): LintFinding[] {
...
export function summarize(findings: LintFinding[]): LintSummary {
  const s: LintSummary = { errors: 0, warnings: 0, infos: 0, total: findings.length }
...
export function fixNegativeDurations(sub: Subtitle, defaultMs = 1000): Subtitle {
  return {
    ...sub,
    cues: sub.cues.map((c) => (c.end <= c.start ? { ...c, end: c.start + defaultMs } : c)),
  }
}
